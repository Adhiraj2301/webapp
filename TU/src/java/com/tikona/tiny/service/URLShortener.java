 ...
 * @author rahul.yadav
 */
import com.tikona.tiny.Dao.DatabaseDao;
...
//		}
        ...
        // System.out.println("Using method " + u.convertToShort("https://www.amazon.in/?tag=googinabkkenshoo-21&ascsubtag=383f7a66-71de-4eb0-8119-bc9486fd2d71&ie=UTF8/"));
    }

    public String convertToShort(String long_url, int ttl,String createdBy,String source) {
        String shortCode = "";
        String status;
        boolean shortyUrl = false,insertStatus=false;
        try {
            URLShortener u = new URLShortener(5, "");
            String urls[] = {long_url};
            for (int i = 0; i < urls.length; i++) {
                System.out.println("URL:" + urls[i] + "\tTiny: "
                        + u.shortenURL(urls[i]) + "\tExpanded: "
                        + u.expandURL(u.shortenURL(urls[i])));
                shortCode = u.shortenURL(urls[i]);
            }


            logger.info("inside Convert2Short "+shortCode);


            DatabaseDao dao = new DatabaseDao();
            status = dao.checkUrlFirst(long_url);

            if (status.length() > 0) {
                shortCode = status;
            } else {

                shortyUrl = dao.checkShortUrl(shortCode);
                if (shortyUrl) {
                    convertToShort(long_url, ttl,createdBy,source);
                } else {
                   insertStatus=dao.insertLongUrl(long_url, shortCode, ttl,createdBy,source);
                   if(!insertStatus)
                   {
                    shortCode="0";
